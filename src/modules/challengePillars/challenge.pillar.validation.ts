  ...
  .superRefine((data, context) => {
    ...
    if (data.name !== expected.name) {
      context.addIssue({
        ...
        path: ['name'],
        message:
          `${data.slug} name must be ${expected.name}`,
      });
    }
    ...
    if (data.icon !== expected.icon) {
      context.addIssue({
        ...
        path: ['icon'],
        message:
          `${data.slug} icon must be ${expected.icon}`,
      });
    }
    ...
    if (data.order !== expected.order) {
      context.addIssue({
        ...
        path: ['order'],
        message:
          `${data.slug} order must be ${expected.order}`,
      });
    }
    ...
    if (
      ...
      context.addIssue({
        ...
        path: ['priceCents'],
        message:
          'Paid pillar requires priceCents or stripePriceId',
      });
    }
...
export const createChallengePillarValidation =
  z.object({
    body: createChallengePillarBodySchema,
  });

export const updateChallengePillarValidation =
  z.object({
    params: z.object({
...
export const challengePillarIdValidation =
  z.object({
    params: z.object({
      id: mongoObjectIdSchema,
    }),
...
export const challengePillarSlugValidation =
  z.object({
    params: z.object({
      slug: z.enum(PILLAR_SLUGS),
    }),
