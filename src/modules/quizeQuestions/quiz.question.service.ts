...
type QuestionConfiguration = {
  ...
  correctBooleanAnswer?: boolean | undefined;
};
...
const validateQuestionConfiguration = ({
  ...
  if (new Set(normalizedOptions).size !== options.length) {
    return throwServiceError("Quiz question options must be unique", 400);
  }

  const uniqueCorrectIndexes = new Set(correctOptionIndexes);

  if (uniqueCorrectIndexes.size !== correctOptionIndexes.length) {
    return throwServiceError("Correct option indexes must be unique", 400);
  }
...
const getAllQuizQuestions = async ({
  ...
  const query = QuizQuestion.find(filter)
    .sort({
      module: 1,
      order: 1,
    })
...
const getQuestionsByModule = async (
  ...
  const questionFilter: QueryFilter<IQuizQuestion> = {
    module: new Types.ObjectId(moduleId),
  };
  ...
  const query = QuizQuestion.find(questionFilter)
    .sort({ order: 1 })
    .populate("createdBy", "fullName email role profileImage")
    .populate("updatedBy", "fullName email role profileImage");
...
const getSingleQuizQuestion = async (
  ...
  assertValidObjectId(questionId, "Quiz question ID");
  ...
  const query = QuizQuestion.findOne(filter)
    .populate({
      path: "module",
      select: "title slug moduleNumber pillar status",
  ...
  assertFound(question, "Quiz question not found", 404);
...
const updateQuizQuestion = async (
  ...
  if (question.status === "archived") {
    throwServiceError("Archived question cannot be updated", 400);
  }
  ...
  } else if (payload.options !== undefined) {
    nextOptions = payload.options;
  } else {
    nextOptions = question.options ? [...question.options] : undefined;
  }

  let nextCorrectOptionIndexes: number[] | undefined;

  if (payload.correctOptionIndexes === null) {
    nextCorrectOptionIndexes = undefined;
  } else if (payload.correctOptionIndexes !== undefined) {
  ...
  let nextCorrectBooleanAnswer: boolean | undefined;

  if (payload.correctBooleanAnswer === null) {
    nextCorrectBooleanAnswer = undefined;
  } else if (payload.correctBooleanAnswer !== undefined) {
  ...
  if (payload.explanation === null) {
    question.set("explanation", undefined);
  } else if (payload.explanation !== undefined) {
...
const publishQuizQuestion = async (questionId: string, actorId: string) => {
  assertValidObjectId(questionId, "Quiz question ID");
  ...
  if (question.status === "archived") {
    throwServiceError("Archived question cannot be published", 400);
  }
  ...
  question.set("archivedAt", undefined);
...
const moveQuizQuestionToDraft = async (questionId: string, actorId: string) => {
  assertValidObjectId(questionId, "Quiz question ID");
  ...
  if (question.status === "archived") {
    throwServiceError("Archived question cannot be moved to draft", 400);
  }
...
const archiveQuizQuestion = async (questionId: string, actorId: string) => {
  assertValidObjectId(questionId, "Quiz question ID");
